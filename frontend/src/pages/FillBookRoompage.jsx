...
const FillBookRoompage = () => {
  ...
  const { selectedTimeSlots, room } = location.state || {};
  ...
  const handleConfirmBooking = () => {
    ...
    setTimeout(() => {
      navigate("/upcoming", { state: newTask });
    }, 1500);
  }
  ...
  const [timeStart, setTimeStart] = useState(["09:00", "10:00", "11:00"])
  const [timeEnd, setTimeEnd] = useState(["10:00", "11:00", "12:00"])
  ...
  return (
    <>
      ...
      <header className="header flex items-center gap-4 mb-6 mt-6 relative">
        <button
        ...
        >
          <img src={ArrowBack} alt="Back" style={{ width: 45, height: 50 }} />
        </button>
      ...
      <div className="Fill-book-room-container text-[#53311C] w-full max-w-3xl mx-auto bg-white rounded-2xl shadow-md p-4 sm:p-6 font-Inter">
        <div className="flex flex-row  gap-6">
          ...
          <div id="A" className="flex-1">
            <h1 className="room-name text-base text-sm text-[#53311C] mb-4" style={{ fontSize: '30px' }}>{room.name}</h1>
            <div className=" grid grid-cols-2 mb-6 text-sm ">
              ...
              <div className="relative">
                ...
                <select style={{ borderColor: '#B37E32', width: '80%', padding: '0.25rem ', color: '#53311C' }} className="block mb-2 border px-10 py-1 rounded focus:border-[#8b5a40] focus:shadow-[0_0_0_3px_rgba(139,90,64,0.1)] focus:bg-white"
                  ...
                  {timeStart
                    .filter(time => parseInt(timeSelectedEnd.split(":")[0], 10) - parseInt(time.split(":")[0], 10) > 0)
                    .map(time => (
                      <option key={time} value={time} style={{ color: '#53311C' }}>
                        {time}
                      </option>
                    ))}
                </select>
              ...
              <div className="relative">
                ...
                <select style={{ borderColor: '#B37E32', width: '80%', padding: '0.25rem ', color: '#53311C' }} className="block mb-2 border px-10 py-1 rounded focus:border-[#8b5a40] focus:shadow-[0_0_0_3px_rgba(139,90,64,0.1)] focus:bg-white"
                  ...
                  {timeEnd
                    .filter(time => parseInt(time.split(":")[0], 10) - parseInt(timeSelectedStart.split(":")[0], 10) > 0)
                    .map(time => (
                      <option key={time} value={time} style={{ color: '#53311C' }}>
                        {time}
                      </option>
            ...
            <div className="mb-2 text-center"><h1 className="text-base text-lg font-bold inline " style={{ color: '#BB8F6E' }}>Total Price</h1> <h1 className="text-lg font-bold inline " style={{ color: '#3C2415' }}></h1>{money * hour} THB</div>
            <div className="flex justify-center">
              <button className="w-full bg-brown-500 text-white py-2 px-4 rounded hover:bg-brown-600 transition duration-300 mt-2 justify-center" style={{ height: "auto", width: "65%" }} onClick={handleConfirmBooking} disabled={isBooking}>
                Confirm Booking
              </button>
            </div>
          </div>
          ...
          <div id="B" className="flex flex-1 flex-col items-center justify-center">
            <img src={room.image} alt="Room" style={{ width: 'auto', height: '70%' }} className="rounded-lg" />
